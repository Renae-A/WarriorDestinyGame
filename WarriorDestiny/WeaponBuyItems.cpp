...
void Game::BuyWeapon()
{
	...
	for (int i = 0; i < 6; i++)	// for every item in the weaponStore
	{
		cout << (i + 1) << ". Quantity: (" << weaponStore.itemList[i].quantity << ") " << weaponStore.itemList[i].name << endl;	// display the quantity, and the name
	}
	cout << endl;
	...
	while (decisionMade1 == false)
	{
		...
		else if (storeItem.quantity < amount)	// if the weapon store does not have the amount of items that the user wants to buy
		{
		...
		{
			cout << "You have purchased " << amount << " " << weaponStore.itemList[decision - 1].name << "/s." << endl;	// displays the amount and item name that the user bought
			storeItem.quantity -= amount;	// remove the amount that the user bought from the quantity the weapon store has
			weaponStore.gold += (storeItem.price * amount);	// add (the price of the item x the amount) to the weapon store's gold 
	...
	{
		WeaponShopMenu();	// call the ArmorShopMenu function
	}
	else	// if the input is invalid
	{
		cout << "You did not enter a valid response. Returning to shop menu." << endl;
