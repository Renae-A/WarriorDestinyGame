...
void Game::SellWeapon()
{
	int decision = 0;

	cout << "~~~~ Sell Items ~~~~" << endl;
	cout << "You have " << character.playerStore.gold << " gold." << endl;	// displays user gold

	cout << "Your Inventory:" << endl; cout << endl;
	for (int i = 0; i < 6; i++)	// for every item in the user inventory (that relates to the items in the weapon store)
	{
		cout << (i + 1) << ". Quantity: (" << character.playerStore.itemList[i].quantity << ") " << character.playerStore.itemList[i].name << endl;	// display the quantity, and the name
	}
	cout << endl;
	...
	while (decisionMade == false)
	{
		...
		{
			WeaponShopMenu();		// call WeaponShopMenu() function to go back
	...
	Item& storeItem = weaponStore.itemList[decision - 1];				// storeItem will be easier to use in code (is the user choice of item in the weapon store's inventory)
	...
	cout << "The shop has " << storeItem.quantity << " " << storeItem.name << "/s." << endl;					// displays the weapon store's quantity of the item
	int amount = 0;
	...
	while (decisionMade1 == false)
	{
		...
		else if (playerItem.price > weaponStore.gold)	// if the weapon store's gold is less than the price of the item (cannot afford)
		{
		...
		else	// if the sale is valid
		{
			...
			storeItem.quantity += amount;	// add the amount the user bought to the quantity of the item the weapon store has
			weaponStore.gold -= (storeItem.price * amount);	// subtract (the price of the item x the amount) from the weapon store's gold
			decisionMade1 = true;	// The user has given valid input, exits loop
		}
	...
	{
		SellWeapon();	// call the SellWeapon function
	}
	else if (YesNoAnswer == 2)	// if the user does not want to sell another item
	{
		WeaponShopMenu();	// call the WeaponShopMenu function
	}
